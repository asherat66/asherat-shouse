...
// ---- 第三代前端锚点（dsh-web-frontend 0.1.2-alpha.1 · index-D-eoFxDP.js）----
// 压缩器再改名：vc→Xl（case 渲染函数，参数 (n,i,o)，流式在 context 参数 o 上）、
// DOM→React 转换 vc/Xu→Aa、style 解析 hp/jd/Sd→Zh、React→d（jsx/Fragment）。
const VC_G3_ORIG = String.raw`for(const f of o.attributes)f.name==="class"?l.className=f.value:f.name==="style"?l.style=Zh(f.value):l[f.name]=f.value;`
const VC_DEF_G3_BEFORE = String.raw`}function Aa(n,i){`
const SCRIPT_FILTER_G3 = String.raw`if(o.localName==="script"||o.localName==="iframe"||o.localName==="object"||o.localName==="embed")return null;`
const CASE_V6_G3 = String.raw`case"html":return function(){if(typeof localStorage!=="undefined"&&localStorage.getItem("dsh.rawHtml")!=="0"){try{var vr=window.__vcpStable&&window.__vcpStable.render(n.value,o.streaming);if(vr!==null&&vr!==undefined)return vr}catch(e){if(typeof console!=="undefined")console.warn("[vcp] html 渲染异常，已回退源码",e)}}else if(typeof console!=="undefined")console.warn("[vcp] html 渲染已关闭（dsh.rawHtml===\"0\"，</> 按钮可重新开启）");return n.value}();`
const VC_V6_G3 = String.raw`for(const f of o.attributes){if(f.name==="onclick"){const m=/^input\s*\(\s*['"]([\s\S]*?)['"]\s*\)\s*;?\s*$/.exec(f.value);if(m)l.onClick=function(){const fn=window.__dshInput;fn&&fn(m[1])};continue}if(/^on/i.test(f.name))continue;if(f.name==="style"){let sv=f.value;sv=sv.replace(/position\s*:\s*fixed\s*;?/gi,"").replace(/z-index\s*:\s*\d{4,}\s*;?/gi,"").replace(/(?<![\w-])content\s*:[^;]*;?/gi,"");let so={};for(const st of sv.split(";")){const ci=st.indexOf(":");if(ci===-1)continue;const k=st.slice(0,ci).trim().replace(/-([a-z])/g,(x,y)=>y.toUpperCase());so[k]=st.slice(ci+1).trim()}l.style=so;l.ref=function(el){if(el)for(const st2 of sv.split(";")){const j=st2.indexOf(":");if(j===-1)continue;const p=st2.slice(0,j).trim(),v=st2.slice(j+1).trim();if(v&&/^(color|font-family|font-size|font-weight|font-style|line-height|letter-spacing|text-align|text-shadow)$/.test(p))el.style.setProperty(p,v,"important")}};continue}if(f.name==="class"){l.className=f.value;continue}if(f.name==="href"&&!/^(https?:|mailto:|\/|#)/i.test(f.value))continue;if(f.name==="src"&&!/^(https?:|data:image\/|\/|#)/i.test(f.value))continue;if((f.name==="action"||f.name==="formaction"||f.name==="xlink:href")&&!/^(https?:|mailto:|\/|#)/i.test(f.value))continue;l[f.name]=f.value}`
...
function main() {
  ...
  // 第三代（0.1.2-alpha.1 · Xl/Aa/Zh 压缩形态）：vc/Xu 均不存在，case 渲染器改名 Xl、
  // DOM→React 转换 Aa、style 解析 Zh、React→d。
  const isGen3 = !isNewFrontend && !t.includes('function vc(n,r){') && t.includes('function Aa(n,i){')
  if (isGen3) console.log('[install-v6] 探测到第三代前端（0.1.2-alpha.1 · Aa 压缩形态），使用第三代锚点组')
  ...
  const caseTarget = isGen3 ? CASE_V6_G3 : isNewFrontend ? CASE_V6_NEW : CASE_V6
  ...
  // 新版前端（rc.8+）与第三代（0.1.2-alpha.1）case"code" 形态未收录：跳过 + 警告，不阻塞安装（不影响基础渲染）。
  if (isNewFrontend || isGen3) {
    console.warn(isGen3 ? '[install-v6] 第三代前端（0.1.2-alpha.1）case"code" 锚点未收录，围栏兜底跳过（不影响基础渲染）'
      : '[install-v6] 新版前端（rc.8+）case"code" 锚点未收录，围栏兜底跳过（不影响基础渲染）')
  ...
  // 2) vc(Xu) 属性循环 → v6
  if (isGen3) {
    // 第三代：原始态单行替换（Aa 内 · f/o/l 变量 · Zh 解析 style）
    next = replaceOnce(next, VC_G3_ORIG, SCRIPT_FILTER_G3 + VC_V6_G3, 'Aa 属性循环(第三代原始态)')
  } else if (isNewFrontend) {
  ...
  const defBefore = isGen3 ? VC_DEF_G3_BEFORE : isNewFrontend ? VC_DEF_NEW_BEFORE : VC_DEF_BEFORE
  const defAfter = isGen3 ? ';function Aa(n,i){' : isNewFrontend ? ';function Xu(n,i){' : ';function vc(n,r){'
  const defLabel = isGen3 ? 'Aa 定义前注入 v6 模块' : isNewFrontend ? 'Xu 定义前注入 v6 模块' : 'vc 定义前注入 v6 模块'
  // 代际标记：三代 0.1.2-alpha.1 起压缩器复用 vc/jd 等短名为无关工具函数，
  // v6-inject 的 typeof 猜名会误命中（function vc(n){...n.apply}）→ parseFrag 全空。
  // 注入时把代际写进 window.__VCP_GEN__，探测函数按代际精确取宿主名。
  const genPrefix = isGen3 ? 'var __VCP_GEN__=3;' : isNewFrontend ? 'var __VCP_GEN__=2;' : 'var __VCP_GEN__=1;'
  next = replaceOnce(next, defBefore, '}' + genPrefix + v6Inject + defAfter, defLabel)
  if (next === null) {
  ...
  const onFilterMark = isGen3 ? 'if(/^on/i.test(f.name))continue;' : isNewFrontend ? 'if(/^on/i.test(d.name))continue;' : 'if(/^on/i.test(c.name))continue;'
  const features = [
  ...
  if (isGen3 && countOccurrences(written, String.raw`for(const f of o.attributes)f.name==="class"`) !== 0) {
    ok = false
    console.error('[install-v6] 特征异常：仍存在原始 Aa 属性循环')
  }

...
// 第三代（0.1.2-alpha.1 · Aa 形态，install-v6 注入的 SCRIPT_FILTER_G3，变量 o）
const ANCHOR_G3 =
  'if(o.localName==="script"||o.localName==="iframe"||o.localName==="object"||o.localName==="embed")return null;'
const TRUSTED_G3 =
  'if(!(typeof window!=="undefined"&&typeof window.__vcpTrusted==="function"&&window.__vcpTrusted())&&(o.localName==="script"||o.localName==="iframe"||o.localName==="object"||o.localName==="embed"))return null;'
...
// 代际探测：老版（vc/i）、新版（Xu/l）、第三代（Aa/o）
const isNewFrontend = t.indexOf('function Xu(n,i){') !== -1 && t.indexOf('function vc(n,r){') === -1
const isGen3 = !isNewFrontend && t.indexOf('function vc(n,r){') === -1 && t.indexOf('function Aa(n,i){') !== -1
const ANCHOR = isGen3 ? ANCHOR_G3 : isNewFrontend ? ANCHOR_NEW : ANCHOR_OLD
const TRUSTED = isGen3 ? TRUSTED_G3 : isNewFrontend ? TRUSTED_NEW : TRUSTED_OLD
if (isNewFrontend) console.log('[trusted] 探测到新版前端（rc.8+ · Xu 压缩形态），使用新锚点组')
if (isGen3) console.log('[trusted] 探测到第三代前端（0.1.2-alpha.1 · Aa 压缩形态），使用第三代锚点组')

// 幂等：已打补丁（vc/Xu 锚点已被可信条件替换）→ 跳过
if (t.indexOf(TRUSTED_OLD) !== -1 || t.indexOf(TRUSTED_NEW) !== -1 || t.indexOf(TRUSTED_G3) !== -1) {
...
try {
  ...
  new vm.Script('function Aa(n,i){const o=n,l={};' + TRUSTED_G3 + '}', { filename: 'trusted-fragment-gen3' })
} catch (e) {

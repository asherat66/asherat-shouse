...
;(function () {
  ...
  // 0.1.2-alpha.1 压缩器再改名 Aa/Zh。
  // 三代 0.1.2-alpha.1 起「typeof 猜名」失效：压缩器复用了 vc/jd 等短名作为无关工具函数
  // （如 function vc(n){return n&&typeof n=="object"&&n.apply}），
  // typeof vc==='function' 会命中它导致 __vcpVc 返回布尔、parseFrag 全空。
  // 修复：由 install-v6.cjs 注入时在模块前设置 window.__VCP_GEN__（1/2/3），
  // 本探测按代际精确取宿主名；无标记时保留 typeof 链兜底（老自装环境）。
  var __gen = typeof window !== 'undefined' && window.__VCP_GEN__ ? window.__VCP_GEN__ : 0
  function __vcpVc() {
    var fn = null
    if (__gen === 3) fn = typeof Aa === 'function' ? Aa : null
    else if (__gen === 2) fn = typeof Xu === 'function' ? Xu : null
    else if (__gen === 1) fn = typeof vc === 'function' ? vc : null
    if (!fn) {
      // 兜底（无标记）：按「参数个数 + 名称形态」排除被污染名——三代 vc(n) 单参，Aa(n,i) 双参
      if (typeof Aa === 'function' && Aa.length === 2 && typeof Zh === 'function') fn = Aa
      else if (typeof Xu === 'function') fn = Xu
      else if (typeof vc === 'function' && vc.length >= 2) fn = vc
    }
    return fn && fn.apply(null, arguments)
  }
  function __vcpHp(s) {
    var fn = null
    if (__gen === 3) fn = typeof Zh === 'function' ? Zh : null
    else if (__gen === 2) fn = typeof jd === 'function' ? jd : (typeof Sd === 'function' ? Sd : null)
    else if (__gen === 1) fn = typeof hp === 'function' ? hp : null
    if (!fn) {
      if (typeof Zh === 'function') fn = Zh
      else if (typeof jd === 'function') fn = jd
      else if (typeof Sd === 'function') fn = Sd
      else if (typeof hp === 'function') fn = hp
    }
    return fn ? fn(s) : {}
  }
  // React 压缩名（跨代际兼容）：rc.5~rc.8 为 f（仍保留）；0.1.2-alpha.1 改为 d（模块导出 $c()，object 含 jsx）。
  function __vcpJsx() {
    var jsx = __gen === 3 ? (typeof d === 'object' && typeof d.jsx === 'function' ? d : null)
      : (typeof f === 'object' && typeof f.jsx === 'function' ? f : null)
    if (!jsx && typeof d === 'object' && typeof d.jsx === 'function') jsx = d
    if (!jsx && typeof f === 'object' && typeof f.jsx === 'function') jsx = f
    return jsx || { jsx: function () { return null }, Fragment: null }
  }
  ...
  function parseFrag(html, isTail) {
    ...
    var res = o.length === 1 ? o[0] : __vcpJsx().jsx(__vcpJsx().Fragment, { children: o })
  ...
  function render(raw, streaming) {
    ...
    if (v.indexOf('>') === -1) return __vcpJsx().jsx(__vcpJsx().Fragment, { children: [] })
    var key = 's:' + v
    ...
    if (F.open) {
      ...
      R = __vcpJsx().jsx(F.open.tag, cProps)
      var outers = []
      for (var j = 0; j < F.outer.length; j++) outers.push(F.outer[j].el)
      if (outers.length > 0) R = __vcpJsx().jsx(__vcpJsx().Fragment, { children: [R].concat(outers) })
    } else {
      var all = []
      for (var m = 0; m < F.outer.length; m++) all.push(F.outer[m].el)
      if (tailEl) all.push(tailEl)
      R = all.length === 0 ? null : (all.length === 1 ? all[0] : __vcpJsx().jsx(__vcpJsx().Fragment, { children: all }))
    }
  ...
  function injectRootChrome(R) {
    ...
    var el = R
    if (el.type === __vcpJsx().Fragment && el.props && Array.isArray(el.props.children) && el.props.children.length) {
  ...
  function attachMathRef(node, streaming) {
    if (!node || typeof node !== 'object') return
    var isEl = (node.type && node.type !== __vcpJsx().Fragment) || !!node.tag
    if (isEl) {
